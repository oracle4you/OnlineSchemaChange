...
if [ $(ls $source_storage_dir/*.atrn 2> /dev/null | wc -l) -gt 0 ]; then
...
mysql -h localhost -N -s -e"call db_manager.dump_entire_partition_account_data_v2('$account','${source_storage_dir}/account_transfer_dump/',$new_partition_id)" > $source_storage_dir/account_transfer_dump/result_dmp.txt
...
touch ${source_storage_dir}/${account}.txt
...
# Prepare partition reorganizer file --> empty on v2 (partitions created after starting entire process)
